...
questions_asked = 0
questions_total = functions.get_number_of_questions(questions)
while correct + incorrect < questions_total:
    functions.show_table(questions)
    user_input = input().title()
    user_data = functions.price_input(user_input)
    if not user_data:
    ...
    if question["asked"]:
        ...
        continue
    functions.print_question(question["question"])
...
functions.show_stats(points, correct, incorrect)
functions.save_results_to_file(points, correct, incorrect)
